...
### Phase Context
Wave 2 — Language Support. This step implements language detection.

### This Step
Implement language detection from filenames per SPEC §3.1 and §4.2.

## Prerequisites
- Step 01 complete (phf dependency available)

## Files to Read Before Starting
- `SPEC.md` §3.1, §4.2 — language detection, supported languages
- `src/lang.rs` — current stub

## Implementation

### Task 1: Implement language detection in src/lang.rs

The language registry maps file extensions and full filenames to tree-sitter language identifiers.
Use `phf` for compile-time perfect hashing.
...
mod tests {
    ...
    fn detect_with_path() {
        assert_eq!(
            detect_language(Path::new("src/lib/main.rs")),
            Some("rust")
        );
        assert_eq!(
...
- [ ] `cargo check` exits 0
- [ ] `cargo test lang` exits 0 (all lang tests pass)
- [ ] `detect_language` returns correct language for: .rs, .py, .ts, .go, .java
- [ ] `detect_language` returns correct language for: Makefile, Dockerfile
- [ ] `detect_language` returns `None` for unknown extensions
- [ ] Uses `phf_map!` for O(1) lookup

## Reviewer Instructions

You are reviewing Step 03. Verify:

1. Run `cargo test lang::tests` — all tests must pass
2. Check `src/lang.rs` contains `EXTENSION_TO_LANG` and `FILENAME_TO_LANG` phf maps
3. Verify `detect_language` checks full filename first, then extension
