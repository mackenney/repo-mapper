...
### Phase Context
Wave 5 — Caching. This step implements persistent tag caching.

### This Step
Implement persistent tag cache with rusqlite + bincode per SPEC §5.

## Prerequisites
- Step 05 complete (Tag type with Serialize/Deserialize)

## Files to Read Before Starting
- `SPEC.md` §5.1–§5.5 — cache purpose, location, key/value, validity, error recovery
- `src/cache/tag_cache.rs` — current stub

## Implementation

### Task 1: Implement TagCache in src/cache/tag_cache.rs
    ...
    pub fn new(root: &Path) -> Self {
        ...
        match Self::open_sqlite(&cache_path) {
            ...
            Err(e) => {
                ...
                warn!("Tag cache open failed ({}); attempting delete and recreate", e);
                ...
                match Self::open_sqlite(&cache_path) {
                    ...
                    Err(e2) => {
                        ...
                        warn!("Tag cache recreation failed ({}); using in-memory fallback", e2);
                        TagCache {
                            conn: None,
                            fallback: HashMap::new(),
                            using_fallback: true,
...
- [ ] `cargo check` exits 0
- [ ] `cargo test tag_cache` exits 0
- [ ] Cache location is `{root}/.aider.tags.cache.v{VERSION}` per SPEC §5.2
- [ ] Cache key is absolute path (SPEC §5.3)
- [ ] Cache validates mtime on lookup (SPEC §5.4)
- [ ] Mtime mismatch returns None (cache miss)
- [ ] SQLite errors fall back to in-memory HashMap (SPEC §5.5)
- [ ] Uses bincode for serialization (not FlatBuffers per resolution R3)

## Reviewer Instructions

You are reviewing Step 08. Verify:

1. Run `cargo test cache::tag_cache::tests` — all tests must pass
2. Check cache path format: `.aider.tags.cache.v{N}` where N is CACHE_VERSION
3. Verify `get` checks mtime with tolerance ~0.001
4. Verify `set` uses `INSERT OR REPLACE` SQL
5. Check fallback mode is triggered on SQLite errors
...
```bash
git checkout HEAD -- src/cache/tag_cache.rs
```
