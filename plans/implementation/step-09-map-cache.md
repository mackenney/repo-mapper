...
### Phase Context
Wave 5 — Caching. This step runs in parallel with tag cache.

### This Step
Implement in-memory map cache with refresh policies per SPEC §11.

## Prerequisites
- None (can be implemented independently)

## Files to Read Before Starting
- `SPEC.md` §5.6, §11 — in-memory map cache, refresh policies
- `src/cache/map_cache.rs` — current stub

## Implementation

### Task 1: Implement MapCache in src/cache/map_cache.rs
...
mod tests {
    ...
    fn cache_auto_mode_threshold() {
        ...
        cache.set(key.clone(), "fast map".to_string(), Duration::from_millis(500));
...
- [ ] `cargo check` exits 0
- [ ] `cargo test map_cache` exits 0
- [ ] Files mode key is 3-tuple (chat, other, max_tokens)
- [ ] Auto mode key is 5-tuple (adds mentioned_fnames, mentioned_idents)
- [ ] Empty collections become None in cache key (SPEC §11)
- [ ] Manual mode returns last_map regardless of key
- [ ] Always mode never uses cache
- [ ] Auto mode uses cache only if last_duration > 1.0s
- [ ] force_refresh bypasses cache reads but writes still occur

## Reviewer Instructions

You are reviewing Step 09. Verify:

1. Run `cargo test cache::map_cache::tests` — all tests must pass
2. Check `MapCacheKey` has `Files` and `Auto` variants
3. Verify `non_empty_sorted` returns None for empty input
4. Check `should_use_cache` implements all 4 mode behaviors
5. Verify `set` always writes to both keyed cache and last_map
...
```bash
git checkout HEAD -- src/cache/map_cache.rs
```
